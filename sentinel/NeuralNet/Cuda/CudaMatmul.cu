...
void CudaMatrix::multiplyPointersInto(const float* left, size_t leftRows, size_t leftCols, const float* right, size_t rightRows, size_t rightCols, float* out, bool transposeLeft, bool transposeRight, float beta) {
    ...
    CudaMatmul::launchSharedMemoryMatmul(left, right, out, static_cast<int>(outRows), static_cast<int>(outCols), static_cast<int>(sharedCount), transposeLeft, transposeRight, nullptr, beta);
}
...
void CudaMatrix::multiplyIntoTimed(const CudaMatrix& left, const CudaMatrix& right, CudaMatrix& out, double& kernelMilliseconds, bool transposeLeft, bool transposeRight) {
    ...
    out.ensureSize(leftRows, rightCols);
    CudaMatmul::launchSharedMemoryMatmul(left.buffer.deviceData, right.buffer.deviceData, out.buffer.deviceData, static_cast<int>(leftRows), static_cast<int>(rightCols), static_cast<int>(leftCols), transposeLeft, transposeRight, &kernelMilliseconds, 0.0f);
}
...
__device__ void CudaMatmul::runSharedMemoryMatmul(const float* left, const float* right, float* out, int rowCount, int columnCount, int sharedCount, bool transposeLeft, bool transposeRight, float beta) {
    ...
    const int outputIndex = outputRow * columnCount + outputColumn;
    if (beta == 0.0f)
        out[outputIndex] = accumulator;
    else
        out[outputIndex] = accumulator + beta * out[outputIndex];
}
...
__global__ void CudaMatmulSharedMemoryEntry(const float* left, const float* right, float* out, int rowCount, int columnCount, int sharedCount, bool transposeLeft, bool transposeRight, float beta) {
    CudaMatmul::runSharedMemoryMatmul(left, right, out, rowCount, columnCount, sharedCount, transposeLeft, transposeRight, beta);
}

bool CudaMatmul::launchCublasLtMatmul(const float* deviceLeft, const float* deviceRight, float* deviceOut, int rowCount, int columnCount, int sharedCount, bool transposeLeft, bool transposeRight, double* kernelMilliseconds, const float* deviceBiasOrNull, float beta) {
    ...
    const cublasStatus_t matmulStatus = cublasLtMatmul(state.handle, matmulDesc, &alpha, deviceLeft, layoutLeft, deviceRight, layoutRight, &cublasBeta, deviceOut, layoutOut, deviceOut, layoutOut, algoPointer, workspacePointer, workspaceSize, CudaMatmul::activeStream());
...
void CudaMatmul::launchSharedMemoryMatmulKernel(const float* deviceLeft, const float* deviceRight, float* deviceOut, int rowCount, int columnCount, int sharedCount, bool transposeLeft, bool transposeRight, double* kernelMilliseconds, float beta) {
    ...
    if (kernelMilliseconds == nullptr) {
        CudaMatmulSharedMemoryEntry<<<gridDimension, blockDimension, 0, CudaMatmul::activeStream()>>>(deviceLeft, deviceRight, deviceOut, rowCount, columnCount, sharedCount, transposeLeft, transposeRight, beta);
...
void CudaMatmul::launchSharedMemoryMatmul(const float* deviceLeft, const float* deviceRight, float* deviceOut, int rowCount, int columnCount, int sharedCount, bool transposeLeft, bool transposeRight, double* kernelMilliseconds, float beta) {
    ...
    if (beta == 0.0f && CudaAmp::launchCublasLtMatmulFp16(deviceLeft, deviceRight, deviceOut, rowCount, columnCount, sharedCount, transposeLeft, transposeRight, kernelMilliseconds)) return;
    if (CudaMatmul::launchCublasLtMatmul(deviceLeft, deviceRight, deviceOut, rowCount, columnCount, sharedCount, transposeLeft, transposeRight, kernelMilliseconds, nullptr, beta)) return;

    CudaMatmul::launchSharedMemoryMatmulKernel(deviceLeft, deviceRight, deviceOut, rowCount, columnCount, sharedCount, transposeLeft, transposeRight, kernelMilliseconds, beta);
}
