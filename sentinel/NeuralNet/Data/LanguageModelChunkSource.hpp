...
/// <summary>
/// streams corpus rows once into compact token-id examples, then serves train chunks without re-encode
...
public:
    ...
    /// <summary>first maxRows train-hash texts (truncated); rewinds the stream</summary>
    std::vector<std::string> prepareTokenizerSample(int maxRows);

    /// <summary>
    /// one corpus pass: encode all train rows into memory, fill test reservoir up to cap
...
private:
    bool tryMakeExample(const CorpusRow& row, LanguageModelExample& out) const;
    ...
    std::unique_ptr<TextRowReader> reader;
    std::string path;
    ...
    std::vector<CorpusRow> pendingRows;
