...
ClassificationDataset ClassificationDataset::build(const std::vector<std::string>& corpus, const BPETokenizer& tokenizer) {
...
ClassificationDataset ClassificationDataset::buildLabeled(const std::vector<std::string>& texts, const std::vector<int>& labels, const BPETokenizer& tokenizer, int classCount) {
    if (texts.size() != labels.size()) throw std::invalid_argument("ClassificationDataset::buildLabeled size mismatch");
