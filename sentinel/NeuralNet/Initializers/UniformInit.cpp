...
Matrix UniformInit::matrix(int rows, int columns, float scale, unsigned seed) {
    ...
    Matrix result(static_cast<size_t>(rows), static_cast<size_t>(columns), 0.0f);
...
void UniformInit::fill(Matrix& matrix, float scale, unsigned seed) {
    ...
    for (size_t row = 0; row < matrix.rows; ++row) {
        for (size_t column = 0; column < matrix.cols; ++column) matrix.at(row, column) = UniformInit::sampleValue(seed, scale);
    }
}
