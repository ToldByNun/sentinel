...
Matrix MeanPool::forward(const Matrix& embeddings) {
    ...
    for (size_t dimension = 0; dimension < embeddingDim; ++dimension) {
        float sum = 0.0f;
        for (size_t tokenIndex = 0; tokenIndex < this->lastSequenceLength; ++tokenIndex) sum += embeddings.data[dimension][tokenIndex];
...
Matrix MeanPool::backward(const Matrix& pooledGradient) const {
    ...
    if (pooledGradient.data.empty() || pooledGradient.data[0].size() != 1) throw std::invalid_argument("MeanPool::backward expects gradient shape embeddingDim x 1");
