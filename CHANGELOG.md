...
- **Python mid-level ops bindings**: `Matrix`, `Softmax` / `SiLU` / `CrossEntropy`, `Adam` / `AdamState` / `SGD`, `Embedding`, `RMSNorm`, `TransformerBlock` (+ caches/grads), `LanguageModelExample` / `LanguageModelGradients` / `LanguageModelCache`. `LanguageModel.forward` / `example_loss` / `accumulate_example` / `apply_gradients` (host step API promoted from private), weight accessors, `train_step` helper, optional `Matrix.to_numpy` / `from_numpy`. Example: [`examples/python/custom_train_loop.py`](examples/python/custom_train_loop.py).
