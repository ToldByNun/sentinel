...
- Expand [`.github/workflows/c-cpp.yml`](.github/workflows/c-cpp.yml) beyond compile-only: C++ `train_tiny`/`generate`, all host `SENTINEL_*_SMOKE` gates, `pip install` + import, and Python host examples (`ci_host_smoke.py`, train/generate/config/custom/HF demo, `train_chunks.py`).

### Added

- **GGUF import/export:** `Gguf::{loadConfig,loadMappedWeights,save}` + `LanguageModel::loadGguf` / `saveGguf` (Python `load_gguf` / `save_gguf`). Llama.cpp tensor names for `llama` / `mistral` / `qwen2`; F32 export; F32/F16/BF16 import (quantized packs rejected). Docs [`docs/gguf.md`](docs/gguf.md); example [`examples/python/gguf_roundtrip.py`](examples/python/gguf_roundtrip.py); smoke `SENTINEL_GGUF_SMOKE=1`.
- **Python streaming first-class:** `LanguageModelChunkSource.iter_train_chunks` / `__iter__`, `take_train_chunk()`, `train_dataset()`, `is_train_row`; GIL release on `materialize` / `prepare_tokenizer_sample` / `train_chunks`. Example [`examples/python/train_chunks.py`](examples/python/train_chunks.py); CI runs it + end-to-end `ci_host_smoke` streaming train. `JsonlLoader` / streaming JSONL accept `text` and `content` in addition to `problem_statement`.
